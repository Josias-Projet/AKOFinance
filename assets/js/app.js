...
async function replaceRemoteTable(table, rows) {
  ...
  const onConflictMap = {
    ...
    businesses: "user_id,name",
    budgets: "user_id,category_name",
    transactions: "id",
  };

  const conflictColumn = onConflictMap[table];
  ...
  const fetchResult = await supabaseClient.from(table).select("id").eq("user_id", userId);
  if (fetchResult.error) throw fetchResult.error;

  const currentIds = (fetchResult.data || []).map(function (r) { return r.id; });

  const keepIds = rows.map(function (r) {
    if (table === "transactions") return r.id;
    return null;
  }).filter(Boolean);

  if (table === "transactions") {
    const toDelete = currentIds.filter(function (id) { return keepIds.indexOf(id) === -1; });
    if (toDelete.length > 0) {
      const deleteResult = await supabaseClient.from(table).delete().in("id", toDelete);
      if (deleteResult.error) throw deleteResult.error;
    }
  } else {
    const keepNames = rows.map(function (r) { return table === "budgets" ? r.category_name : r.name; });

    const nameField = table === "budgets" ? "category_name" : "name";
    const fetchNamesResult = await supabaseClient.from(table).select("id," + nameField).eq("user_id", userId);
    if (fetchNamesResult.error) throw fetchNamesResult.error;

    const toDelete = (fetchNamesResult.data || [])
      .filter(function (r) { return keepNames.indexOf(r[nameField]) === -1; })
